...
func TestWeatherHandlerSuccess(t *testing.T) {
	...
	handler := weatherHandler(&CepService{BaseURL: viaCep.URL, HTTPClient: http.DefaultClient},
...
func TestWeatherHandlerCEPNOTFOUND(t *testing.T) {
	...
	handler := weatherHandler(&CepService{BaseURL: viaCep.URL, HTTPClient: http.DefaultClient}, nil)
	req := httptest.NewRequest(http.MethodGet, "/?cep=12345678", nil)
	rec := httptest.NewRecorder()
	handler(rec, req)
